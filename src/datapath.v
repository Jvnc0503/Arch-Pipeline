module datapath(
  ...
  // ================= FETCH =================
  wire [31:0] PCNextF, PCPlusStepF, PCInputF, PCTargetE;  
  
  mux2 #(32)  pcmux(.d0(PCPlusStepF), .d1(PCTargetE), .s(PCSrcE), .y(PCNextF));
  ...
  pipereg r_if_id_pc4   (.clk(clk), .reset(reset), .en(~StallD), .clr(PCSrcE), .d(PCPlusStepF),  .q(PCPlusStepD));
  ...
  pipereg r_id_ex_pc4 (.clk(clk), .reset(reset), .en(1'b1), .clr(FlushE), .d(PCPlusStepD), .q(PCPlus4E));
  ...
  always @(posedge clk or posedge reset) begin
   ...
   always @* begin
       case(funct3E)
           3'b000:  TakeBranchE = ZeroE;       
           3'b001:  TakeBranchE = ~ZeroE;      
           3'b100:  TakeBranchE = LtE;         
           3'b101:  TakeBranchE = ~LtE;        
